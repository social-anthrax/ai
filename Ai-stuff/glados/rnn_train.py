...
# rnn stuff temporarily in contrib, moving back to code in TF 1.1
...
#
SEQLEN = 30
BATCHSIZE = 100  # seems to be the sample size
...
codetext, valitext, bookranges = txt.read_data_files(
    shakedir, validation=False)
...
loss = tf.nn.softmax_cross_entropy_with_logits(
    logits=Ylogits, labels=Yflat_)  # [ BATCHSIZE x SEQLEN ]
...
for x, y_, epoch in txt.rnn_minibatch_sequencer(codetext, BATCHSIZE, SEQLEN, nb_epochs=20):
    ...
    if step % _50_BATCHES == 0:
        feed_dict = {X: x, Y_: y_, Hin: istate, pkeep: 1.0,
                     batchsize: BATCHSIZE}  # no dropout for validation
        y, l, bl, acc, smm = sess.run(
            [Y, seqloss, batchloss, accuracy, summaries], feed_dict=feed_dict)
        txt.print_learning_learned_comparison(
            x, y, l, bookranges, bl, acc, epoch_size, step, epoch)
    ...
    if step % _50_BATCHES == 0 and len(valitext) > 0:
        ...
        vali_x, vali_y, _ = next(txt.rnn_minibatch_sequencer(
            valitext, bsize, VALI_SEQLEN, 1))  # all data in 1 batch
    ...
    if step // 3 % _50_BATCHES == 0:
        ...
        for k in range(1000):
            ryo, rh = sess.run([Yo, H], feed_dict={
    ...
    if step // 10 % _50_BATCHES == 0:
        saved_file = saver.save(
            sess, 'checkpoints/rnn_train_' + timestamp, global_step=step)
